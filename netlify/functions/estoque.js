...
exports.handler = async (event) => {
  ...
  try {
    const { data: tipos, error: errTipos } = await supabase.from('tipos_combustivel').select('*').order('nome');
    if (errTipos) throw errTipos;

    const { data: entradas, error: errEnt } = await supabase.from('entradas_combustivel').select('tipo_combustivel_id, litros');
    if (errEnt) throw errEnt;

    const { data: saidas, error: errSai } = await supabase.from('lancamentos').select('tipo_combustivel_id, litros');
    if (errSai) throw errSai;

    const totalEntradas = {};
    entradas.forEach((e) => {
      const k = e.tipo_combustivel_id || 'sem_tipo';
      totalEntradas[k] = (totalEntradas[k] || 0) + (Number(e.litros) || 0);
    });

    const totalSaidas = {};
    saidas.forEach((s) => {
      const k = s.tipo_combustivel_id || 'sem_tipo';
